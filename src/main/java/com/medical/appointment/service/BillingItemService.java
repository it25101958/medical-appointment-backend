...
public class BillingItemService {
    ...
    public BillingItemResponse createBillingItem(BillingItemRequest request) {
        ...
        if (request.getLabTestId() != null) {
            LabTest labTest = labTestRepository.findById(request.getLabTestId())
                    .orElseThrow(() -> new RuntimeException(
                            "LabTest not found with id: " + request.getLabTestId()));
    ...
    public void deleteBillingItem(Integer id) {
        if (!billingItemRepository.existsById(id))
            throw new RuntimeException(
                    "Cannot delete: BillingItem not found with id: " + id);
    ...
    private BillingItemResponse convertToResponse(BillingItem item) {
        ...
        if (item.getLabTest() != null)
            response.setLabTestId(item.getLabTest().getLabTestId());
        return response;
    }
}